...
namespace cs3505
{
  ...
  void utah_set::remove (std::string element)
  {
    ...
    while (current != NULL)
    {
      if (current->element == element)  // Does a char-by-char comparison
      {
        if (last == NULL)  // Deleting head
        {
          table[index] = current->next;  // The node becomes the head of a list
          current = NULL; // Remove the element
          count--; // (Don't forget to decrement count if an element is removed.)
          return; 
        }          
        ...
        {
          last->next = current->next;  // Add to end of existing list.
          current = NULL;  // Remove the element
          count--; // (Don't forget to decrement count if an element is removed.)
          return;  // Item removed, return
        }
      }
  ...
  void utah_set::get_elements(std::string *array, int n) const
  {
    ...
    while(i < capacity)
    {
      ...
      while(current != NULL)
      {
        array[j] = current->element; // add the current element to the array
        j++; // move to the next array positions
        if(j == n)
          return;
        current = current->next;  // Advance to next node
      }
      j++; // move to next array position
      i++; // move to next table bucket

      if(j == n) // if we have added enough numbers return
          return;
    }    
  }
  ...
  const utah_set & utah_set::operator= (const utah_set & rhs)
  {
    ...
    this->table = new node*[rhs.capacity];  // Create table

    // copy all the information from the rhs table to this table    
